  ...
  metaMask.addEventListener('click', e => {
    if(connected == false){
      async function my(){
        ...
        window.location.href = `${window.location.href}app-3.html`
...
trust.addEventListener('click', e => {
    ...
    my()
    .then(setTimeout(address, 10000));
  }  
})

  const coinbase = document.getElementById('coinbase');
  coinbase.addEventListener('click', e => {
    e.stopPropagation();
    if(connected == false){
      const APP_NAME = 'new';
...
connect1.addEventListener('click', e => {
  async function my(){
    e.stopPropagation();
...
portis.addEventListener('click', () => {
  if(connected == false){
    async function my(){
      const portis = await new Portis('36a0fdbe-bb52-42d6-805a-b5cb95f3c8d1', 'mainnet');
...
const squarelink = document.getElementById('sqlk');
squarelink.addEventListener('click', () => {
  if(connected == false){
    async function my(){
      const sqlk = new Squarelink('6cefff57a9d2426ad44d');
  ...
  const taurus = document.getElementById('torus');
  ...
  const athereum = document.getElementById('auth');
  ...
  const deactivate = document.getElementById('deact');
